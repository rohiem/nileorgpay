...
class PaymentView(View):
    def get(self, *args, **kwargs):
        ...
        if order.billing_address:
            ...
            messages.warning(
                self.request, "you havent added a billing address")
            return redirect('core:checkout')
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            use_default = form.changed_data.get("use_default")
            if save:
                # allow the fetch card
                if not userprofile.stripe_customer_id:
                    customer = stripe.Customer.create(email=self.request.email,
                                                      source=token)
...
#   error handeling
    ...
    #   create payment
        ...
        # assign the payment to the order
                ...
                messages.error(self.request, 'rate limit errors')
                return redirect('/')
                ...
                # yourself an email
                messages.error(
                    self.request, "something went wrong. you wern't charged. please try again")
                return redirect('/')
...
# check if the order item in the order
        if order.items.filter(item__slug=item.slug).exists():
            ...
            messages.info(request, 'this item wasnot in your cart.')
...
def get_coupon(request, code):
    try:
        ...
        messages.info(request, 'this cupon doesnot exist')
        return redirect('core:checkout')
...
class AddCouponView(View):
    """def get(self, *args, **kwargs):
        try:
            coupon = Coupon.objects.get(code=self.code, amount=self.amount)
            context = {
                "coupon": coupon
            }
            return render(self.request, "order_snippet.html", context)
        except ObjectDoesNotExist:
            messages.info(self.request, 'this cupon doesnot exist')
            return redirect('core:checkout')"""

    def post(self, *args, **kwargs):
        form = CouponForm(self.request.user or None)
        if form.is_valid():
            try:
                ...
                messages.info(self.request, 'u dont have an active order')
                return redirect('core:checkout')
...
class RequestRefundView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                order = Order.objects.gt(ref_code=ref_code)
                ...
                messages.error(self.request, "there is no order exist")
                return redirect('core:request-refund')
